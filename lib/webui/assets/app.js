...
(function () {
  ...
  function renderTree(tree) {
    ...
    barePage = !!tree.bare;
    // Edge/Chrome often ignore --window-size for app windows when the
    // browser is already running; enforce the page's intended size once.
    if (tree.bare && tree.size && !window.__webuiSized) {
      window.__webuiSized = true;
      try { window.resizeTo(tree.size[0], tree.size[1]); } catch (e) { /* ignore */ }
    }
