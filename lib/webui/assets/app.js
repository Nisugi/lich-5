...
(function () {
  ...
  // App-mode windows and script-opened popups hide the browser toolbar;
  // regular tabs never do. Geometry reporting/enforcement and self-closing
  // only make sense for standalone windows - a maximized tab must never
  // overwrite a floating window's remembered geometry.
  function standaloneWindow() {
    try { return !window.toolbar.visible; } catch (e) { return false; }
  }
  ...
  function renderTree(tree) {
    ...
    // uses the final size.
    if ((tree.size || tree.position) && !window.__webuiSized && standaloneWindow()) {
  ...
  function reportGeometry() {
    if (!standaloneWindow() || !currentPage || !wsReady) return;
