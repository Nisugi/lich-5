...
(function () {
  ...
  function connect() {
    ...
    ws.onclose = function () {
      ...
      if (barePage && !document.getElementById('bare-disconnected')) {
  ...
  // Bare pages are floating app windows: close ourselves; when the browser
  // refuses (multi-entry history), fall back to the landing page. Regular
  // tabs just go back to the landing page.
  function closePage() {
    if (barePage) {
      try { window.close(); } catch (e) { /* ignore */ }
      setTimeout(function () {
  ...
  function renderTree(tree) {
    ...
    document.body.classList.toggle('bare-page', !!tree.bare);
    barePage = !!tree.bare;
    // Edge/Chrome often ignore --window-size/--window-position for app
    // windows when the browser is already running; enforce the page's
    // intended geometry once. resize before move so clamping-to-screen
    // uses the final size.
    if (tree.bare && (tree.size || tree.position) && !window.__webuiSized) {
      window.__webuiSized = true;
      if (tree.size) { try { window.resizeTo(tree.size[0], tree.size[1]); } catch (e) { /* ignore */ } }
      if (tree.position) { try { window.moveTo(tree.position[0], tree.position[1]); } catch (e) { /* ignore */ } }
    }
    morphChildren(body, tree.children || []);
  }

  /* ---------- window geometry reporting (bare pages) ----------
   * Bare pages are floating app windows; report their outer geometry so the
   * owning script can persist it and reopen the window where it was.
   */
  var barePage = false;
  var lastGeometry = '';
  function reportGeometry() {
    if (!barePage || !currentPage || !wsReady) return;
