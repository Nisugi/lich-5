  ...
  var factories = {
    ...
    textarea: {
      create: function (node) {
        ...
        ta.rows = node.rows_hint || 4;
        ta.value = node.value || '';
        if (node.placeholder) ta.placeholder = node.placeholder;
        // fire on blur (change), like text_input - multi-line means Enter
        // inserts a newline rather than committing.
        ta.addEventListener('change', function () { emit(node.cid, 'change', ta.value); });
        return labelWrap(node, ta);
      },
      patch: function (root, node) {
        root.querySelector('.c-label').textContent = node.label || '';
        var ta = root.querySelector('textarea');
        ta.rows = node.rows_hint || 4;
        if (!isEditing(root)) ta.value = node.value || '';
      }
    },
    ...
    image_button: {
      create: function (node) {
        ...
        b.type = 'button';
        var img = document.createElement('img');
        img.className = 'c-ib-img';
        img.draggable = false;
        img.loading = 'lazy';
        b.appendChild(img);
        var cap = document.createElement('span');
        cap.className = 'c-ib-label';
        b.appendChild(cap);
        b.addEventListener('click', function () {
          if (b.disabled) return;
          emit(node.cid, 'click', null);
        });
        factories.image_button.patch(b, node);
        return b;
      },
      patch: function (root, node) {
        root.className = 'c-image-button' +
          (node.selected ? ' selected' : '') +
          (node.disabled ? ' disabled' : '');
        root.disabled = !!node.disabled;
        if (node.tooltip) { root.title = node.tooltip; } else { root.removeAttribute('title'); }
        var img = root.querySelector('.c-ib-img');
        if (img && img.getAttribute('src') !== node.src) img.src = node.src;
        if (img) {
          img.alt = node.label || node.tooltip || '';
          img.style.width = node.w ? node.w + 'px' : '';
          img.style.height = node.h ? node.h + 'px' : '';
        }
        var cap = root.querySelector('.c-ib-label');
        if (cap) {
    ...
    grid: {
      // A uniform CSS grid: every column has equal width across all rows,
      // so matrices (e.g. the escort from x to grid) line up. Cells hold
      // arbitrary child nodes.
      create: function (node) {
        var g = document.createElement('div');
        g.className = 'c-grid' + (node.compact ? ' compact' : '');
        g.style.gridTemplateColumns = 'repeat(' + (node.cols || 1) + ', minmax(0, 1fr))';
        (node.children || []).forEach(function (cellNode) {
          var cell = document.createElement('div');
          cell.className = 'c-cell';
          cell.dataset.cid = cellNode.cid;
          cell.dataset.type = 'cell';
          morphChildren(cell, cellNode.children || []);
          g.appendChild(cell);
        });
        return g;
      },
      patch: function (root, node) {
        root.className = 'c-grid' + (node.compact ? ' compact' : '');
        root.style.gridTemplateColumns = 'repeat(' + (node.cols || 1) + ', minmax(0, 1fr))';
        var cells = node.children || [];
        if (root.children.length !== cells.length) {
          root.textContent = '';
          cells.forEach(function (cellNode) {
            var cell = document.createElement('div');
            cell.className = 'c-cell';
            cell.dataset.cid = cellNode.cid;
            cell.dataset.type = 'cell';
        ...
        cells.forEach(function (cellNode, i) {
          morphChildren(root.children[i], cellNode.children || []);
        });
      }
    },
