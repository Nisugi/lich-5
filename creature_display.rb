...
module CreatureDisplay
  ...
  DEFAULT_CONFIG = {
    ...
    'colors' => {
      ...
      'progress_healthy' => '#4CAF50', # Green for healthy HP
      'progress_damaged' => '#FF5722', # Red for damaged HP
  ...
  class << self
    ...
    def setup_signals
      ...
      @@window.signal_connect('configure-event') do |w, _|
    ...
    def update_target_bar(target)
      ...
      status_text = build_status_text(creature)
      
      # Format: "Name (curr/max) [SIW]"
      full_text = "#{truncated_name} #{hp_text}#{status_text}"
      escaped_text = CGI.escape_html(full_text)
      
      # Use center alignment with truncated names and configurable font color
      font_color = (@@config['colors'] && @@config['colors']['font']) || '#FFFFFF'
      label.set_markup("<span color='#{font_color}' weight='bold'>#{escaped_text}</span>")
      label.halign = :center
    ...
    def build_hp_text(target, creature)
      if creature && creature.max_hp && creature.max_hp > 0
        current_hp = [creature.current_hp || 0, 0].max
    ...
    def build_status_text(creature)
      ...
      # Convert status effects to abbreviations
      abbrevs = creature.status.map do |status|
        STATUS_ABBREV[status.to_s.downcase] || status.to_s[0].upcase
      end.compact
      
      abbrevs.empty? ? "" : " [#{abbrevs.join}]"
    end
    ...
    def update_bar_color(bar, fraction)
      ...
      color = if fraction <= 0.3
        colors['progress_damaged'] || '#FF5722'  # Red for low HP
      else
        colors['progress_healthy'] || '#4CAF50'  # Green for healthy HP
      end
    ...
    def stop
      ...
      # Kill the script when stop is called
      Thread.new do
